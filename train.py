...
def train(**kwargs):
    ...
    logger.info("Start Training: Total epochs:{} Total iterations:{} Batch_Size:{} Dataset:{} Training_Size:{} device:{}".format(epochs,all_iter, batch_size,dataset, dt_size,device))
    for epoch in range(epochs):
        ...
        for sample in data_loader:
            ...
            if types=="sp":
                ...
                writer.add_scalar("csc_loss",model.csc_loss,now_iter)
            elif types=="IFVD":
        ...
        if  types=="csc":
            logger.info("第{}个Epoch:total loss={} CrossEntropyLoss = {} CSC loss ={}".format(epoch+1, epoch_loss/step,epoch_ce_loss/step,epoch_csc_loss/step))
        elif types=="IFVD":
...
def main():
    ...
    assert args.mode in ["train", "test","test_all","train_paraphraser"]
    if args.mode == "train":
        train(
